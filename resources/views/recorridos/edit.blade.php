<x-app-layout :show-footer="false">
  {{-- SortableJS para reordenar panoramas --}}
  <x-slot name="head">
      ...
      document.addEventListener('DOMContentLoaded', () => {
        ...
        if (btn) {
          btn.addEventListener('click', () => {
            const ids = Array.from(document.querySelectorAll('#lista-orden [data-id]')).map(li => li.dataset.id);
  ...
  <div class="py-6">
    <div class="max-w-3xl px-4 mx-auto">
      ...
      <form method="POST" action="{{ route('recorridos.update', $recorrido) }}" class="overflow-hidden bg-white border rounded-lg shadow-sm border-slate-200">
        ...
        <div class="p-5 space-y-5">
          ...
          <div class="flex justify-between pt-2">
            <x-ui.btn-ghost href="{{ route('recorridos.index') }}">Volver</x-ui.btn-ghost>
      ...
      {{-- Orden de panoramas --}}
      <div class="mt-6 overflow-hidden bg-white border rounded-lg shadow-sm border-slate-200">
        <div class="p-5">
          <div class="flex items-center justify-between mb-3">
            <h3 class="text-lg font-semibold">Orden de panoramas</h3>
            <x-ui.btn-primary id="btn-guardar-orden">Guardar orden</x-ui.btn-primary>
          </div>
